...
def read_smiles(dataset_name, data_path, target, task):
    ...
    print(pkl_path)
    print('len(text_embedding_data)',len(text_embedding_data))
    return smiles_data, text_embedding_data, labels


class MolTestDataset(Dataset):
    def __init__(self, dataset_name, data_path, target, task):
        super(Dataset, self).__init__()
        self.smiles_data,self.text_embedding_data, self.labels = read_smiles(dataset_name, data_path, target, task)
        self.task = task
    ...
    def __getitem__(self, index):
        ...
        print('--------')
        print(x.shape)
        x = torch.tensor(self.text_embedding_data[index])
        x = x.squeeze(-1).repeat(len(type_idx),1)
...
class MolTestDatasetWrapper(object):
    ...
    def __init__(self, 
        task_name, batch_size, num_workers, valid_size, test_size, 
    ...
    def get_data_loaders(self):
        train_dataset = MolTestDataset(dataset_name=self.task_name.lower(),data_path=self.data_path, target=self.target, task=self.task)
