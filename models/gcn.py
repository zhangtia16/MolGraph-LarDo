...
class GCNConv(MessagePassing):
    def __init__(self, hid_dim, aggr="add"):
        super(GCNConv, self).__init__()
        self.hid_dim = hid_dim
        self.aggr = aggr

        self.weight = Parameter(torch.Tensor(hid_dim, hid_dim))
        self.bias = Parameter(torch.Tensor(hid_dim))
...
class GCN(nn.Module):
    def __init__(self, num_layer=5, hid_dim=300, feat_dim=256, drop_ratio=0, pool='mean'):
        ...
        for layer in range(num_layer):
            self.gnns.append(GCNConv(hid_dim, aggr="add"))
